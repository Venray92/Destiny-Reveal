...
- Isi tiap hasil masih dummy placeholder utk 5 sistem contoh (Zodiak, Shio,
  Weton, Numerologi, Matrix Destiny) — bukan hasil perhitungan asli, karena
  engine shio/numerologi/matrix_destiny masih stub (lihat progress-notes.md).
- Sistem lain di luar 5 itu (BaZi, Zi Wei, Human Design, MBTI, Big Five,
  Enneagram, DISC, Golongan Darah, Love Language, Tarot) belum punya konten
  dummy panjang — amplopnya tetap bisa dibuka tapi isinya jujur bilang
...
def _inject_style():
    st.markdown(
        ...
        .rp-detail-card-wrap { flex-shrink: 0; width: 220px; display: flex; flex-direction: column;
            align-items: center; margin: 0 auto; }
        .rp-detail-card { width: 220px; height: 300px; border-radius: 18px; padding: 7px;
            background: linear-gradient(155deg, #f3d488, #c9a227 45%, #8a6a12 55%, #f3d488);
            box-shadow: 0 22px 45px -20px rgba(139,90,47,0.5); }
        .rp-detail-card-inner { width: 100%; height: 100%; border-radius: 14px; padding: 3px;
            background: linear-gradient(155deg, #fdf0c8, #c9a227); }
        .rp-detail-card-art { width: 100%; height: 100%; border-radius: 11px;
            background: radial-gradient(circle at 30% 22%, #fbead0, transparent 55%),
            linear-gradient(150deg, #f3b56a, #c9683a 55%, #7a3d22);
            display: flex; align-items: center; justify-content: center; }
        .rp-detail-tagline { margin-top: 12px; font-size: 10.5px; letter-spacing: 0.07em;
...
def _render_opened_badge(system):
    st.markdown(
...
def render():
    ...
    # ── Detail hasil yang sudah dibuka, sesuai urutan dibuka ──
